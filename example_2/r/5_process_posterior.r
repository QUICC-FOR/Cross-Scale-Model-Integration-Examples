...
parser$add_argument("-g", "--glm", help="an rds file with a glm of the initial guess at the naive model")
...
glmmod = readRDS(argList$glm)
model = read.csv(argList$infile, header=FALSE, stringsAsFactors=FALSE)
modelPosterior = prep_posterior(model, sort(names(glmmod$coefficients)), argList$burnin, argList$thin)
