...
process_output = function(posteriorSample, newData, SE = TRUE, credInterval = TRUE, allreps = FALSE) {
	...
	# allReps: if true, will output the prediction for each MCMC replicate, otherwise just summary statistics
	...
	calcAllReps = (SE || credInterval || allreps)
	...
	if(calcAllReps) predictedPrPresenceReps = inv_logit(predictedPrPresenceReps)
...
make_model_text = function(vars, responseName = "weightedPresence", weightName = "weightedN") {
	mt = c(
		"model {",
		"for(i in 1:N) {",
		paste(responseName, "[i] ~ dbin(pr[i], ", weightName, "[i])", sep=""),
		paste(linear_predictor(vars), sep=""),
		"}",
		prior(vars),
		"}")
